...
ASN MOD: Add method to return all keys under a section
*/
...
public:
    ...
    // Return the list of keys found in a section
...
inline const std::set<std::string> INIReader::Keys(const std::string& section) const
{
    std::set<std::string> keys;
    for (auto it = _values.begin(); it != _values.end(); it++)
    {
        if (it->first.rfind(section, 0) == 0)  // starts with section name
        {
            std::string value = it->first.substr(section.size()+1);
            keys.insert(value);
        }
    }
    return keys;
